                ...
                <div class="card card-stats card-round">
                    <div class="card-header">
                        <div class="d-flex align-items-center">
                            ...
                            <a href="{{route('cabang.create')}}" class="btn btn-primary btn-round btn-md ml-auto text-white">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            ...
                            <tbody>
                                ...
                                $id = 1;
                                @endphp
                                @forelse($cabang as $row)
                                <tr>
                                    <td>{{ $id++}}</td>
                                    <td>{{ $row->kode_cabang}}</td>
                                    <td>{{ $row->wilayah_cabang}}</td>
                                    <td>{{ $row->alamat_cabang}}</td>
                                    <td>{{ $row->editor}}</td>
                                    <td>
                                            <a href="{{ route('cabang.edit', $row->id)}}" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a>
                                            <form action="{{ route('cabang.destroy', $row->id)}}" method="post" class="d-inline">
                                                @csrf
                                                @method('delete')
                                                <button class="btn btn-danger btn-sm" onclick="return confirm('Apakah anda ingin menghapus data? {{$row->wilayah_cabang}}')"">
