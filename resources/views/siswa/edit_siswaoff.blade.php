        ...
        <div class="row">
            <div class="col-sm-6 col-md-12">
                <div class="card card-stats card-round">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{route('siswa-off.update', $siswaOFF->id)}}">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="nama_siswa">Nama Lenkap</label>
                                <input autocomplete="off" type="text" class="form-control" id="text" name="nama_siswa" value="{{$siswaOFF->nama_siswa}}" readonly>
                            </div>
                            <div class="form-group">
                                <label for="nama_tentor">Nama Tentor</label>
                                <input autocomplete="off" type="text" class="form-control" id="text" name="nama_tentor" value="{{$siswaOFF->nama_tentor}}" readonly>
                            </div>
                            <div class="form-group">
                                <label for="tgl_join">Tanggal Berhenti</label>
                                <input autocomplete="off" type="text" class="form-control tgl" name="tgl_off" value="{{$siswaOFF->tgl_off}}">
                            </div>
                            <div class="form-group">
                                <label for="status">Status</label>
                                <input autocomplete="off" type="text" class="form-control" id="text" name="keterangan" value="{{$siswaOFF->keterangan}}">
                            </div>
                            <div class="form-group">
