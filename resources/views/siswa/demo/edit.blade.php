...
@section('content')
            ...
            <div class="row">
                <div class="col-sm-6 col-md-12">
                    <div class="card card-stats card-round">
                        ...
                        <div class="card-body">
                            <form method="post" action="{{ route('Demo-Siswa.update', $murid->id) }}">
                                @csrf
                                @method('PUT')
                                <div class="form-group">
                                    <label for="kode_siswa">Absen Siswa</label>
                                    <input autocomplete="off" type="text" class="form-control" id="text" name="kode_siswa"
                                        value="{{ $murid->kode_siswa }}" readonly>
                                </div>
                                <div class="form-group">
                                    <label for="nama_siswa">Nama Lenkap</label>
                                    <input autocomplete="off" type="text" class="form-control" id="text" name="nama_siswa"
                                        value="{{ $murid->nama_siswa }}">
                                </div>
                                <div class="form-group">
                                    <label for="lahir">Tanggal Lahir</label>
                                    <input autocomplete="off" type="text" class="form-control tgl" id="text" name="lahir"
                                        value="{{ $murid->lahir }}">
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <input autocomplete="off" type="text" class="form-control" id="text"
                                        name="asal_sekolah" value="{{ $murid->asal_sekolah }}">
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <input autocomplete="off" type="text" class="form-control" id="text" name="jadwal_les"
                                        value="{{ $murid->jadwal_les }}">
                                </div>
                                <div class="form-group">
                                    <label for="nama_tentor">Nama Tentor</label>
                                    <select name="nama_tentor" id="" class="form-control">
                                        @foreach ($tentor as $row)
                                            <option value="{{ $row->nama_tentor }}"
                                                {{ $murid->nama_tentor == $row->nama_tentor ? 'selected' : '' }}>
                                                {{ $row->nama_tentor }} - ({{ $row->status }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label for="">Jumlah Pertemuan</label>
                                    <select name="" id="pilih_paket">
                                        <option>Pilih Paket</option>
                                        @foreach ($paket as $row)
                                            <option value="{{ $row->pertemuan }}">{{ $row->paket }}</option>
                                        @endforeach
                                    </select>
                                    <div class="form-group">
                                        <label for="">*Edit Data Jumlah Pertemuan</label>
                                        <input type="text" class="form-control" id="data_pertemuan"
                                            name="jumlah_pertemuan" readonly>
                                    </div>
                                    <div class="form-group">
                                        <label for="">*Database Jumlah Pertemuan</label>
                                        <input type="text" class="form-control" id=""
                                                value="{{ $murid->jumlah_pertemuan }}" readonly>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="pembayaran">Pembayaran</label>
                                    <select name="" id="pilih_data">
                                        @foreach ($paket as $row)
                                            <option value="{{ $row->harga }}"> {{ $row->kategori }} -
                                                {{ $row->paket }} - {{ $row->kelas }}</option>
                                        @endforeach
                                    </select>
                                    <div class="form-group">
                                        <label for="">*Edit Data Pembayaran</label>
                                        <div class="input-group mb-3">
                                            <div class="input-group-prepend">
                                                <span class="input-group-text" id="basic-addon3">Rp.</span>
                                            </div>
                                            <input type="text" class="form-control" id="data_harga" name="pembayaran"
                                                readonly>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="">*Database Pembayaran</label>
                                        <div class="input-group mb-3">
                                            <div class="input-group-prepend">
                                                <span class="input-group-text" id="basic-addon3">Rp.</span>
                                            </div>
                                            <input type="text" class="form-control" id=""
                                                value="{{ $murid->pembayaran }}" readonly>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="tgl_join">Tanggal Bergabung</label>
                                    <input autocomplete="off" type="text" class="form-control tgl" name="tgl_join"
                                        value="{{ $murid->tgl_join }}">
                                </div>
                                <div class="form-group">
                                    <label for="no_hp">No.hp/Whatsapp</label>
                                    <input autocomplete="off" type="text" class="form-control" id="text" name="no_hp"
                                        value="{{ $murid->no_hp }}">
                                </div>
                                <div class="form-group">
                                    <label for="alamat_rumah">Alamat</label>
                                    <input autocomplete="off" type="text" class="form-control" id="text"
                                        name="alamat_rumah" value="{{ $murid->alamat_rumah }}">
                                </div>
                                <div class="form-group">
                                    <label for="wali_siswa">Nama Wali Murid</label>
                                    <input autocomplete="off" type="text" class="form-control" id="text" name="wali_siswa"
                                        value="{{ $murid->wali_siswa }}">
                                </div>
                                <div class="form-group">
                                    <label for="sumber_info">Sumber Informasi</label>
                                    <input autocomplete="off" type="text" class="form-control" id="text"
                                        name="sumber_info" value="{{ $murid->sumber_info }}">
                                </div>
                                <div class="form-group">
    ...
    <script>
        var data_paket = document.getElementById("pilih_paket");
        data_paket.onchange = function() {
            var data_pertemuan = document.getElementById("data_pertemuan");
            data_pertemuan.value = this.value;
        };
    </script>

    <script>
        var pilih_data = document.getElementById("pilih_data");
        pilih_data.onchange = function() {
            var data_harga = document.getElementById("data_harga");
            data_harga.value = this.value;
        };
    </script>
@endsection
