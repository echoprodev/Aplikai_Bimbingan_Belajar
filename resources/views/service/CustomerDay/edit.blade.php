                ...
                <div class="card card-stats card-round">
                    <div class="card-header">
                        <div class="d-flex align-items-center">
                            <h4 class="card-title">Form Edit Kualitas Pelayanan (Custemer)</h4>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{route('Customer-Day.update', $custemer->id)}}">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="">Nama Siswa</label>
                                <input type="text" class="form-control" id="data-nama" name="nama_siswa" value="{{$custemer->nama_siswa}}" autocomplete="off" readonly placeholder="Input Nama Lengkap">
                            </div>
                            <div class="form-group">
                                <label for="">Nama Tentor</label>
                                <input type="text" class="form-control" id="tentor" name="nama_tentor" value="{{$custemer->nama_tentor}}" readonly placeholder="Input Nama Lengkap">
                            </div>
                            <div class="form-group">
                                <label for="nama_tentor">Informasi</label>
                                <input type="text" class="form-control" value="{{$custemer->informasi}}" name="informasi">
                            </div>
                            <div class="form-group">
